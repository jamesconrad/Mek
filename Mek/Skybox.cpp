...
{
	Program::getInstance().createShader("skybox", GL_VERTEX_SHADER, "skybox.vert");
	Program::getInstance().createShader("skybox", GL_FRAGMENT_SHADER, "skybox.frag");
...
void Skybox::render()
{
	...
	glBindVertexArray(_vao);
	Program::getInstance().setUniform("skybox", "camera", Camera::getInstance().matrix());
