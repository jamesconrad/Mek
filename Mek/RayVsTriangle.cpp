...
bool RayVsTriangle(glm::vec3 ray, glm::vec3 origin, glm::vec3 p0, glm::vec3 p1, glm::vec3 p2, rvtResponse* out)
{
	...
	if (det > -NEAR_ZERO && det < NEAR_ZERO)	//If det is negative the triangle is backfacing and should be ignored //changed by Jordan from just det < NEAR_ZERO
	...
	if (out->distance >= 0.f) //if statement here added by jordan.
		return true;
...
//	if (out->distance > NEAR_ZERO) //if statement here added by jordan.
//				return true;
//
//		return false;
//}